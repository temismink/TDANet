...
class TDANet(BaseModel):
    def __init__(
        ...
        upsampling_depth=4,
        enc_kernel_size=21,
        num_sources=4,  # Changed from 2 to 4 sources
        sample_rate=16000,
    ):
        ...
        self.num_sources = num_sources  # Adjusted to handle 4 sources
        ...
        # Updated mask_conv to handle 4 sources
        mask_conv = nn.Conv1d(out_channels, num_sources * self.enc_num_basis, 1)
        self.mask_net = nn.Sequential(nn.PReLU(), mask_conv)

        # Back end
        self.decoder = nn.ConvTranspose1d(
            in_channels=self.enc_num_basis * num_sources,  # Adjusted for 4 sources
    ...
    def forward(self, input_wav):
        ...
        x = self.mask_net(x)
        x = x.view(x.shape[0], self.num_sources, self.enc_num_basis, -1)  # Adjusted for 4 sources
    ...
    def get_model_args(self):
        model_args = {"n_src": 4}  # Adjusted to 4 sources
        return model_args
